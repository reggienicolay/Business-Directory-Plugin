...
add_action(
	...
	function () {
		...
		if ( is_page_template( 'templates/directory.php' ) || is_singular( 'business' ) || has_shortcode( get_post()->post_content ?? '', 'business_filters' ) ) {
			...
			wp_enqueue_script(
				'bd-geolocation',
				plugins_url( 'assets/js/geolocation.js', __DIR__ ),
				array( 'jquery', 'bd-filters' ),
				'1.0.0',
				true
			);
			...
			wp_enqueue_script(
				'bd-map',
				plugins_url( 'assets/js/directory-map.js', __DIR__ ),
				$map_deps,
				'1.0.0',
				true
			);
			...
			wp_enqueue_style(
				'bd-map-markers',
				plugins_url( 'assets/css/map-markers.css', __DIR__ ),
				array(),
				'1.0.0'
			);
		}
