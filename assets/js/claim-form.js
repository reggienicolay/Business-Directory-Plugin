...
(function ($) {
	...
	$(document).ready(function () {
		...
		$(document).on('click', '.bd-claim-btn, .bd-btn[href*="Claim"]', function (e) {
			e.preventDefault();
			openModal();
		});

		// Close modal
		$('.bd-claim-close, .bd-claim-modal-overlay').on('click', function () {
			closeModal();
		});
		...
		$('#claim-proof-files').on('change', function (e) {
			handleFileSelection(e.target.files);
		});

		// Form submission
		$('#bd-claim-form').on('submit', function (e) {
			e.preventDefault();
			submitClaim();
		});
	 ...
	 */
	function openModal() {
		$('#bd-claim-modal').fadeIn(200, function () {
	...
	function renderFileList() {
		...
		selectedFiles.forEach(function (file, index) {
			const $fileItem = $('<div class="bd-file-item">' +
		...
		// Handle file removal
		$('.bd-file-remove').on('click', function () {
			...
			success: function (response) {
				if (response.success) {
					...
					setTimeout(function () {
						...
						$('.bd-claim-btn, .bd-btn[href*="Claim"]').each(function () {
							$(this).replaceWith(
