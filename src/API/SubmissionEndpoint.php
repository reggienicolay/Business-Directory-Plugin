...
class SubmissionEndpoint {
    ...
    public function submit_business($request) {
        try {
            ...
            if (!empty($params['address'])) {
                ...
                require_once plugin_dir_path(dirname(__FILE__)) . 'Search/Geocoder.php';
            ...
            if (!empty($params['submitter_name']) || !empty($params['submitter_email'])) {
                update_post_meta($business_id, 'bd_submitter', [
    ...
    private function send_admin_notification($business_id, $params) {
        ...
        $subject = sprintf('[%s] New Business Submission: %s', 
            get_bloginfo('name'), 
            $business_name
        );
