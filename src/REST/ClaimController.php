...
class ClaimController {
    ...
    public static function register() {
        ...
        register_rest_route('bd/v1', '/claim', [
            'methods' => 'POST',
        ...
        register_rest_route('bd/v1', '/claims', [
            'methods' => 'GET',
            'callback' => [__CLASS__, 'get_claims'],
            'permission_callback' => [__CLASS__, 'admin_permission'],
        ]);
        
        // Approve claim (admin only)
        register_rest_route('bd/v1', '/claims/(?P<id>\d+)/approve', [
            'methods' => 'POST',
        ...
        register_rest_route('bd/v1', '/claims/(?P<id>\d+)/reject', [
            'methods' => 'POST',
    ...
    public static function get_claims($request) {
        ...
        if ($status === 'pending') {
            ...
            $claims = []; // TODO: Add get_by_status method if needed
        }
    ...
    private static function notify_claimant_rejected($claim, $notes) {
        ...
        $message = sprintf(
            ...
            $claim['claimant_name'],
            get_the_title($claim['business_id']),
            $notes ?: 'Additional verification is required.',
