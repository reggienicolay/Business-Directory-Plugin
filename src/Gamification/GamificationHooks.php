...
class GamificationHooks {
    ...
    public function __construct() {
        // Review actions
        add_action('bd_review_approved', [$this, 'on_review_approved'], 10, 1);
        add_action('bd_review_created', [$this, 'on_review_created'], 10, 2);
        
        // Helpful votes
        add_action('wp_ajax_bd_mark_helpful', [$this, 'handle_helpful_vote']);
        add_action('wp_ajax_nopriv_bd_mark_helpful', [$this, 'handle_helpful_vote']);
        
        // List creation (when you build this feature)
        add_action('bd_list_created', [$this, 'on_list_created'], 10, 2);
        
        // Business claim
        add_action('bd_business_claimed', [$this, 'on_business_claimed'], 10, 2);
        
        // Profile completion
        add_action('profile_update', [$this, 'check_profile_completion'], 10, 1);
        
        // New user registration
        add_action('user_register', [$this, 'on_user_register'], 10, 1);
        
        // Badge notifications
        add_action('bd_badges_earned', [$this, 'notify_badges_earned'], 10, 2);
    }
    ...
    public function on_review_approved($review_id) {
        ...
        $first_today = $wpdb->get_var($wpdb->prepare("
            SELECT COUNT(*)
            FROM $reviews_table
            WHERE user_id = %d
            AND status = 'approved'
    ...
    public function handle_helpful_vote() {
        ...
        global $wpdb;
        $helpful_table = $wpdb->prefix . 'bd_review_helpful';
        $reviews_table = $wpdb->prefix . 'bd_reviews';
        ...
        if ($already_voted) {
            wp_send_json_error(['message' => 'You already marked this helpful']);
        }
        ...
        if ($review['user_id'] == $user_id) {
            wp_send_json_error(['message' => 'You cannot vote for your own review']);
        }
    ...
    public function on_user_register($user_id) {
        ...
        if ($user_count <= 100) {
            ...
            $wpdb->insert(
                ...
                [
                    'user_id' => $user_id,
                    'badges' => wp_json_encode(['founding_member']),
                    'badge_count' => 1,
                    'rank' => 'newcomer',
    ...
    private function send_badge_email($user, $new_badges) {
        $badge_names = array_map(function($key) {
