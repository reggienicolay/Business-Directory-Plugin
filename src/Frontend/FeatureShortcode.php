...
class FeatureShortcode {
	...
	public static function render_shortcode( $atts ) {
		$atts = shortcode_atts( array(
			'id' => '', 'ids' => '', 'layout' => 'card', 'columns' => '3',
			'show' => 'image,title,rating,excerpt,category,cta', 'cta_text' => 'View Details',
			'source' => '', 'class' => '',
		), $atts, 'bd_feature' );

		$ids = array();
		if ( ! empty( $atts['id'] ) ) $ids = array( absint( $atts['id'] ) );
		elseif ( ! empty( $atts['ids'] ) ) $ids = array_map( 'absint', explode( ',', $atts['ids'] ) );
		$ids = array_filter( $ids );
		if ( empty( $ids ) ) return '<!-- BD Feature: No valid IDs -->';

		$show = array_map( 'trim', explode( ',', $atts['show'] ) );
		self::enqueue_assets();

		$businesses = self::get_businesses( $ids, $atts['source'] );
		if ( empty( $businesses ) ) return '<!-- BD Feature: No businesses found -->';

		switch ( $atts['layout'] ) {
			case 'list': return self::render_list( $businesses, $show, $atts );
			case 'inline': return self::render_inline( $businesses, $show, $atts );
			case 'mini': return self::render_mini( $businesses, $atts );
			default: return self::render_card( $businesses, $show, $atts );
		}
	}

	private static function get_businesses( $ids, $source = '' ) {
		if ( empty( $source ) || self::is_local( $source ) ) return self::get_local( $ids );
	...
	private static function get_local( $ids ) {
		...
		foreach ( $ids as $id ) {
			$post = get_post( $id );
			if ( ! $post || $post->post_status !== 'publish' || $post->post_type !== 'bd_business' ) continue;
			...
			$businesses[] = array(
				'id' => $id, 'title' => $post->post_title, 'excerpt' => $excerpt, 'permalink' => get_permalink( $id ),
	...
	private static function get_remote( $ids, $source ) {
		$source = rtrim( $source, '/' );
		if ( strpos( $source, 'http' ) !== 0 ) $source = 'https://' . $source;
		...
		if ( ! isset( $data['businesses'] ) ) return array();
	...
	private static function render_card( $businesses, $show, $atts ) {
		...
		foreach ( $businesses as $b ) {
			...
			if ( in_array( 'title', $show ) ) $html .= '<h3 class="bd-feature-title"><a href="' . esc_url( $b['permalink'] ) . '">' . esc_html( $b['title'] ) . '</a></h3>';
			if ( in_array( 'rating', $show ) && ! empty( $b['rating'] ) ) {
				$html .= '<div class="bd-feature-rating">' . self::stars( $b['rating'] );
				if ( ! empty( $b['review_count'] ) ) $html .= '<span class="bd-feature-review-count">(' . intval( $b['review_count'] ) . ')</span>';
				$html .= '</div>';
			}
			if ( in_array( 'excerpt', $show ) && ! empty( $b['excerpt'] ) ) $html .= '<p class="bd-feature-excerpt">' . esc_html( $b['excerpt'] ) . '</p>';
			if ( in_array( 'category', $show ) ) {
				$meta = array();
				if ( ! empty( $b['price_level'] ) ) $meta[] = esc_html( $b['price_level'] );
				if ( ! empty( $b['categories'] ) ) $meta[] = esc_html( $b['categories'][0] );
				if ( ! empty( $b['city'] ) ) $meta[] = esc_html( $b['city'] );
				if ( $meta ) $html .= '<div class="bd-feature-meta">' . implode( ' • ', $meta ) . '</div>';
			}
			if ( in_array( 'cta', $show ) ) $html .= '<a href="' . esc_url( $b['permalink'] ) . '" class="bd-feature-cta">' . esc_html( $atts['cta_text'] ) . ' →</a>';
			$html .= '</div></div>';
		}
		return $html . '</div>';
	}
	...
	private static function render_list( $businesses, $show, $atts ) {
		...
		foreach ( $businesses as $b ) {
			...
			if ( in_array( 'title', $show ) ) $html .= '<h4 class="bd-feature-list-title"><a href="' . esc_url( $b['permalink'] ) . '">' . esc_html( $b['title'] ) . '</a></h4>';
			if ( in_array( 'excerpt', $show ) && ! empty( $b['excerpt'] ) ) $html .= '<p class="bd-feature-list-excerpt">' . esc_html( wp_trim_words( $b['excerpt'], 15, '...' ) ) . '</p>';
			if ( in_array( 'category', $show ) && ! empty( $b['categories'] ) ) $html .= '<span class="bd-feature-list-category">' . esc_html( $b['categories'][0] ) . '</span>';
			$html .= '</div><div class="bd-feature-list-meta">';
			if ( in_array( 'rating', $show ) && ! empty( $b['rating'] ) ) $html .= '<div class="bd-feature-list-rating">' . self::stars( $b['rating'] ) . '</div>';
			if ( in_array( 'cta', $show ) ) $html .= '<a href="' . esc_url( $b['permalink'] ) . '" class="bd-feature-list-cta">View →</a>';
			$html .= '</div></div>';
		}
		return $html . '</div>';
	}
	...
	private static function render_inline( $businesses, $show, $atts ) {
		...
		foreach ( $businesses as $b ) {
			...
			if ( in_array( 'rating', $show ) && ! empty( $b['rating'] ) ) $item .= ' <span class="bd-feature-inline-rating">' . self::stars( $b['rating'], true ) . '</span>';
			$items[] = $item . '</span>';
		}
	...
	private static function render_mini( $businesses, $atts ) {
		$items = array();
		foreach ( $businesses as $b ) $items[] = '<a href="' . esc_url( $b['permalink'] ) . '" class="bd-feature-mini">' . esc_html( $b['title'] ) . ' →</a>';
	...
	private static function stars( $rating, $compact = false ) {
		...
		if ( $compact ) return '<span class="bd-stars-compact">' . number_format( $rating, 1 ) . ' ★</span>';
