...
class RegistrationHandler
{
    ...
    {
        add_action('admin_post_nopriv_ll_register_user', [$this, 'handle_registration']);
        add_action('user_register', [$this, 'claim_past_reviews'], 10, 1);
        add_action('wp_login', [$this, 'check_unclaimed_reviews'], 10, 2);
    }
    ...
    public function check_unclaimed_reviews($user_login, $user)
    {
        ...
        $unclaimed_count = $wpdb->get_var($wpdb->prepare("
            SELECT COUNT(*)
            FROM $reviews_table
            WHERE author_email = %s
            AND (user_id IS NULL OR user_id = 0)
    ...
    public static function show_claim_notice()
    {
        ...
        if ($claimed_count || $show_notice) {
            $count = $claimed_count ?: $show_notice;
?>
